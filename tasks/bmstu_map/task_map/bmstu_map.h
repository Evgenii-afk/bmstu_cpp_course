 ...
 *    - operator++() - переход к следующему элементу in-order обходе
...
class avl_balanced_tree
{
   ...
   private:
	...
	uint8_t heightOfTree(tree_node<K, V>* t) { return t ? t->height : 0; }
	...
	void updateHeight(tree_node<K, V>* t)
	{
		...
		{
			t->height =
				1 + std::max(heightOfTree(t->left), heightOfTree(t->right));
		}
	}

	int getBalanceFactor(tree_node<K, V>* t)
	{
		if (t == nullptr)
		{
			return 0;
	...
	void balance(tree_node<K, V>*& t)
	{
		...
		if (balance_factor > 1 && getBalanceFactor(t->left) >= 0)
		{
		...
		if (balance_factor < -1 && getBalanceFactor(t->right) <= 0)
		{
...
class map
{
   public:
											   ...
											   std::bidirectional_iterator_tag>
	{
		...
		typename iterator::value_type* pair_cache_;

		iterator() : current_(nullptr), pair_cache_(nullptr) {}

		explicit iterator(tree_node<K, V>* root, bool is_end = false)
			: current_(nullptr), pair_cache_(nullptr)
		{
			...
			if (!stack_.empty())
			{
				current_ = stack_.top();
				stack_.pop();
				pair_cache_ = new typename iterator::value_type(
					std::piecewise_construct,
					std::forward_as_tuple(current_->key),
					std::forward_as_tuple(current_->value));
			}
		}

		iterator(const iterator& other)
			: current_(other.current_), stack_(other.stack_)
		{
			if (other.current_ != nullptr && other.pair_cache_ != nullptr)
			{
				pair_cache_ = new typename iterator::value_type(
					std::piecewise_construct,
					std::forward_as_tuple(other.current_->key),
					std::forward_as_tuple(other.current_->value));
			}
			else
			{
				pair_cache_ = nullptr;
